...
Proxy.prototype.listen = function(options, callback) {
  ...
  ca.create(this.sslCaDir, function(err, ca) {
    ...
    self.httpServer.on('connection', socket => { socket.on('error', self._onError.bind(self, 'SOCKET_ERROR', null)); });
...
Proxy.prototype._onWebSocketServerConnect = function(isSSL, ws, upgradeReq) {
  ...
  if (upgradeReq.url == '' || /^\//.test(upgradeReq.url)) {
    ...
    if (!hostPort) {
      ctx.clientToProxyWebSocket.emit('error', 'No host header in upgrade request');
      return;
    }
...
Proxy.prototype._onHttpServerRequest = function(isSSL, clientToProxyRequest, proxyToClientResponse) {
  ...
  if (!hostPort) {
    ctx.clientToProxyRequest.emit('error', 'No host header found in request');
    return;
  }
  var headers = {};
...
Proxy.prototype._onWebSocketFrame = function(ctx, type, fromServer, data, flags) {
  ...
  async.forEach(this.onWebSocketFrameHandlers.concat(ctx.onWebSocketFrameHandlers), function(fn, callback) {
    ...
    if (destWebSocket.readyState === WebSocket.OPEN) {
      switch(type) {
        case 'message': destWebSocket.send(data, flags, function(err) {
          if (err) {
            self._onWebSocketError(ctx, err);
          }
        });
        break;
        case 'ping': destWebSocket.ping(data, false, function(err) {
          if (err) {
            self._onWebSocketError(ctx, err);
          }
        });
        break;
        case 'pong': destWebSocket.pong(data, false, function(err) {
          if (err) {
            self._onWebSocketError(ctx, err);
          }
        });
